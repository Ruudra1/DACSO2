...
def eda_analysis():
    ...
    # st.write("This automated EDA shows a high-level analysis of your data, with just a few clicks!")
    # st.write("The sample datasets below have their own unique attributes which highlights several key concepts.")
    st.write('## Data Input')
    #read_file()
    st.info('NOTE: You can also upload your own CSV data through the **<Upload your own CSV data>** option below')
    option = st.selectbox(
    ...
    if st.button('3. Analysis on missing values'):
        missing_values = missing_values_table(df)
        st.write("### Missing value rows:")
        st.write(missing_values)

    if st.button('4. Visualize missing values'):
    ...
    st.write("You can manually change the categorical and numeric variables")
    ...
    if st.button(" View Categorical columns"):
...
if __name__ == "__main__":
    ...
    st.write("# DACSO - Data Acumen Solutions")
    eda_analysis()