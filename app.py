...
st.title("DACSO - Data Acumen Solutions")
if choice == "Home":
    intro.hello()
    ...
    if st.sidebar.checkbox("Login"):
        ...
        if result:
            if username ==  'admin':
                page = st.selectbox("Choose a page", ["Data Exploration","Machine Learning","Admin Page"])
                ...
                #     ml_webapp.ml_analysis()
                    ...
                    cleandb = pd.DataFrame(result,columns=["Username","Password"])
                    st.dataframe(cleandb)
                elif page =="Admin Page":
                    user_result = view_all_users()
                    cleandb = pd.DataFrame(user_result,columns=["Username","Password"])
                    st.dataframe(cleandb)
            else:
                st.success("Logged in as {}".format(username))
                page = st.selectbox("Choose a page", ["Data Exploration","Machine Learning","User Profile"])
                if page=="Data Exploration":
                    eda_webapp.eda_analysis()
                # elif page=="Machine Learning":
                #     #st.info('As a side note: There is an amazing EDA platform to look at from the menu on the left')
